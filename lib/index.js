...
function genrate(ast, analysis) {
  ...
  function traverse(node, currentRef) {
    ...
    switch (node.type) {
      case 'Fragment': {
        ...
        let ref = `${currentRef}`;
        const children = node.children.forEach((children, index) => {
          ref += `.${index === 0 ? 'firstChild' : 'nextSibling'}`;
          return traverse(children, ref);
        });
        console.log({ children });
        return children.join('s');
      }
      case 'Element': {
        let tag = `<${node.name} `;
        console.log({ attrList: node.attributes });
        const attr = node.attributes.forEach((attribute) =>
        ...
        tag += children.join('s') + `</${node.name}>`;
        return tag;
      }
      case 'Attribute': {
        let attribute = '';
        if (node.name.startsWith('on:')) {
          code.create.push(`${currentRef}.$$click=${node.value.value}`);
          code.event.add(node.name.slice(3));
        } else if (node.value.type === 'Expression') {
          code.create.push(
            `_runtime$.bind(() => ${currentRef}.(${node.name}, ${node.value.value}))`
          );
        } else {
  ...
  code.create.push(`const _temp$= _template(${traverse(ast.html, '_el$')})`);
