...
while (i <= content.length) {
  ...
  // result.push(js);
  if (val) {
    const [js, template] = val;
    result =
      template + result.slice(0, startIndex) + js + result.slice(endIndex + 1);
  }
}
...
function genrate(ast, analysis) {
    ...
    switch (node.type) {
      case 'Fragment': {
        ...
        const children = node.children.map((children, index) => {
          return traverse(children, `${currentRef}[${index}]`);
        });
      ...
      case 'Attribute': {
        ...
        if (node.name.startsWith('on:')) {
          ...
          code.mount.push(
            `window._runtime$.bind(() => ${currentRef}.setAttribute("${node.name}", ${node.value.value}))`
          );
        } else {
      ...
      case 'Expression': {
        // code.mount.push(`_runtime$.replace(${currentRef},() => ${node.value})`);
        code.replace.set(currentRef, `() => ${node.value}`);
        return '<!>';
      }
      case 'Text': {
  ...
  if (template) {
    let replace = '';
    if (code.replace.size) {
      replace = `window._runtime$.replace([${[...code.replace.entries()]}])`;
    }
    return [
      `(() => {
        ${code.mount.join('\n')}
      ...
      `export const _temp$= window._runtime$.template('${template}')\n`,
    ];
  }
}
