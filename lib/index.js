...
if (!fs.existsSync("public/chunck")) {
    fs.mkdirSync("public/chunck");
}
...
while (i <= content.length) {
    ...
    if (val) {
        ...
        fs.writeFileSync(`public/chunck/app${i}.js`, result.slice(0, startIndex) + hydration + result.slice(endIndex + 1), 'utf-8')
        console.log(result.slice(0, startIndex) + hydration + result.slice(endIndex + 1));
        result =
...
function genrate(ast, analysis) {
    ...
    if (template) {
        ...
        return [
            ...
            `(() =>{
                const _el$ = document.getElementById("${id}").nextSibling
                ${code.client.join('\n')}
            })()`
        ];
    }
