...
function addRuntime() {
    ...
    fs.writeFileSync('public/runtime.js', addition, 'utf-8');
}
...
while (i <= content.length) {
    ...
    if (val) {
        ...
        fs.writeFileSync(`public/chunck/${id}.js`, result.slice(0, startIndex) + hydration + result.slice(endIndex + 1), 'utf-8')
        result =
        ...
        switch (node.type) {
            ...
            case 'Attribute': {
                ...
                if (node.name.startsWith('on:')) {
                    code.client.push(`${currentRef}.$$${node.name.slice(3)}=${node.value.value};\n console.log(${currentRef})`);
